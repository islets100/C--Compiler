...
GRAMMAR_EXE = $(GRAMMAR_DIR)/compiler_test.exe
...
# 编译语法分析器
grammar:
	cd $(GRAMMAR_DIR) && $(CXX) $(CXXFLAGS) main.cpp GrammarAnalyzer.cpp SLRTable.cpp Parser.cpp ReductionSequenceLogger.cpp -o compiler_test.exe

# 清理
clean:
	cd $(LEX_DIR) && del /Q Lex_Analysis.exe 2>nul || rm -f Lex_Analysis.exe
	cd $(GRAMMAR_DIR) && del /Q compiler_test.exe 2>nul || rm -f compiler_test.exe
...
# 运行语法分析器（需要先运行词法分析器）
run-grammar: grammar
	cd $(GRAMMAR_DIR) && compiler_test.exe
